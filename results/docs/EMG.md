...
可以重点说明四个工程判断：窗口不能跨越标签边界；重叠窗口必须按手势段分组；标准化参数只能来自训练集；Early Stopping按验证损失选择checkpoint。消融实验显示FFT贡献约3.15个百分点，小波贡献较小，说明你是依据实验选择特征，而不是为了堆叠算法。最后主动提出按受试者划分作为下一步。
